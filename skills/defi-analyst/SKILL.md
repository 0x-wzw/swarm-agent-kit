---
name: defi-analyst
description: DeFi research and analysis via Tavily MCP, GeckoTerminal API, and DeFiLlama. Use for protocol research, TVL tracking, yield analysis, token discovery, and competitive landscape research. Prerequisites: Tavily API key (free tier works), curl, jq. Configure mcporter with tavily MCP server.
---
...
```bash
# Compare yields acrossDEXes
curl -s "https://api.llama.fi/overview/dex?exclude_bridge=true" | jq '.dexes[0:5]'
```

## Use Cases

### Research a Protocol Before Investing

```bash
# 1. TVL + metrics
TVL=$(curl -s "https://api.llama.fi/protocol/your-protocol" | jq '.tvl')
# 2. Recent news via Tavily
mcporter call tavily.tavily_search query="protocol-name audit exploit update 2026" max_results=5
# 3. Competitor comparison
mcporter call tavily.tavily_search query="protocol-name vs aave vs compound defi comparison" max_results=3
```
...
- GeckoTerminal free tier: 30 req/min, no auth needed
- DeFiLlama: public API, rate limit ~60 req/min
- Tavily: 20 req/min free tier, 1000 req/month free
- Combine all three for complete protocol analysis
